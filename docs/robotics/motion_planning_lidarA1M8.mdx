...
- `sensor_mounting_angle`: The sensor_mounting_angle refers to the angle between the LIDAR sensor’s scanning plane and the horizontal plane of your robot or platform. This parameter is crucial for accurately interpreting the sensor’s data within your system’s coordinate framework.

NOTE: sensor_mounting_angle could be read from the LIDAR physically and here is how to read it: (Original image from https://docs.donkeycar.com/assets/lidar_angle.png)
![sensor_mounting_angle](/assets/lidar_angle.png)

- `angles_blanked`: This isn’t a standard parameter in the official RPLIDAR documentation or SDK. However, in various applications and software integrations, it often refers to specific angular sectors where LIDAR data is intentionally ignored or filtered out.
