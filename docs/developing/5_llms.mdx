...
api/core/{provider}/chat/completions methods=["POST"]   // single agent
api/core/agent                       methods=["POST"]   // multi agent 
api/core/agent/memory                methods=["DELETE"] // multi agent memory wipe
api/core/rag                                            // RAG knowledge base
api/core/agent/medical               methods=["POST"]   // a healthcare focused multi agent system 
```

## Single-Agent LLM Integration

For testing and introductory educational purposes, we integrate with multiple language models (LLMs) to provide chat completion via a `post /{provider}/chat/completions` endpoint. Each LLM plugin takes fused input data (the `prompt`) and sends it to an LLM. The response is then parsed and provided to `runtime/cortex.py` for distribution to the system actions:
